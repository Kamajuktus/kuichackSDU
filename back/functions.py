    ...
    if repeated == False:
        completion = client.chat.completions.create(
            ...
            messages=[
                {
                "role": "user",
                "content": f"""Сгенерируй 1 корректный тестовый вопрос с 4 вариантами ответа по теме {subject}. 
                Только один ответ должен быть правильным. Также дай три вида подсказок. 
                Первая подсказка указывает на принцип, которым можно воспользоваться, чтобы дать ответ на вопрос. 
                Вторая подсказка укажет на логиеское размышление, приводящее к ответу, но без самого ответа. 
                Третья подсказка должна содержать ответ и полное пояснение логического размышления, которое привело к ответу. 
                Вопрос должен быть на русском языке. 
                Вопрос обязательно должно предворять слово 'Вопрос', 
                Подсказки - словом 'Подсказки', при этом первая словами "Принцип:", вторая - "Логическое размышление:", третья - "Ответ и пояснение:", Ответ - словом 'Ответ', Ответы - должны быть от буквы A до буквы D и ииметь закрывающую скобку после буквы, и в конце обязательно напиши 'Конец'. 
                {difficulty}"""
                }
            ]
        )
        ...
        if conversation_turns:
            ...
            completion = client.chat.completions.create(
                ...
                messages=[
                    {
                    "role": "user",
                    "content": f"""Сгенерируй 1 корректный тестовый вопрос, который будет подобен этому вопросу: '{random_turn}', по теме {subject} с 4 вариантами ответа. 
                    Только один ответ должен быть правильным. Также дай три вида подсказок. 
                    Первая подсказка указывает на принцип, которым можно воспользоваться, чтобы дать ответ на вопрос. 
                    Вторая подсказка укажет на логиеское размышление, приводящее к ответу, но без самого ответа. 
                    Третья подсказка должна содержать ответ и полное пояснение логического размышления, которое привело к ответу.
